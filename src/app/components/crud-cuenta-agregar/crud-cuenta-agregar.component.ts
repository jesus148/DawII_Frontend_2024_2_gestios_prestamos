...
import { FormBuilder, FormControl, FormsModule, Form, ReactiveFormsModule, Validators } from '@angular/forms';
...
export class CrudCuentaAgregarComponent {
  ...
  formRegistrar = this.formBuilder.group({
    validaNumero: ['', [Validators.required, Validators.pattern('^[0-9]{20}$')], this.validaNumeroCuenta.bind(this)],
...
registrar(){
  ...
  validaNumeroCuenta(control: FormControl) {
    return this.cuentaService.validaNumeroCuenta(control.value).pipe(
      ...
      catchError(() => of({ existenNumero: true })) // En caso de error, asumimos que el número ya existe
    );
  }
